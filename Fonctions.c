...
char *DateDeNaissance()
{
    ...
    /* g�n�rer le jours */
    if ((annee % 4 != 0) && (mois == 2))
        jours = rand() % (28 - 1) + 1; // ann�e non bissextile
    if ((annee % 4 == 0) && (mois == 2))
        jours = rand() % (29 - 1) + 1; // ann�e bissextile
...
void AfficherEntete(char *nomFichier)
{
    ...
    printf("\n\t\t+------------------------------------------------------------------+");
    printf("\n\t+------------------------------------------------------------+---------------------+");
    printf("\n\t|Numero du premier bloc (Tete de la liste)                   |      %6d         |", Entete(F, 1));
    printf("\n\t|Nombre d'enreg dans le fichier                              |      %6d         |", Entete(F, 2));
    printf("\n\t|Numero du bloc queue = Nombre de blocs dans le fichier      |      %6d         |", Entete(F, 3));
    printf("\n\t+------------------------------------------------------------+---------------------+");
    Fermer(F);
}
...
void RemplirIndexFichier(char *nomFichier, TableIndex *tabIndex)
{
    ...
    printf(">> En train de remplir la table d'index a partir du fichier, \n Veuillez patienter SVP...! "); 
    while (!stop)
    {
        for (j = 0; j < buf.nb; j++)
        {
...
int InsererEnreg(char *nomFichier, TableIndex *tabIndex)
{
   ...
   inserer: printf("\n\t>> Veuillez inserer un matricule entre 111111 et 999999: ");
    ...
    stop = Faux; 
    while(buf->tab[j].matricule>999999 ||  buf->tab[j].matricule<111111) { 
    printf("\n\t>> Veuillez respecter la condition Svp !\n >> Veuillez inserer un nouveau matricule entre 111111 et 999999: ");
    scanf("%d", &(buf->tab[j].matricule));
    }
    ...
    if (trouv)
    {
        printf("\n   >> Inserttion impossible, le matricule existe deja");
        stop = Vrai;
    }
    if (stop) goto inserer; 
...
void AfficherSelonGrades(char *nomFichier)
{
    ...
    Fermer(F);
    printf("\n\n\t\t+-----------------------------------------------------------+");
    printf("\n\t\t| Le cout de cette opeartion est : %4d accees disque       |", cout);
    printf("\n\t\t+-----------------------------------------------------------+");
}
...
void SupprimSelonMatricule(TableIndex *tabIndex, char *nomFichier)
{
    ...
    scanf("%d", &a);
    if (a == 1)
        AffichageBloc(nomFichier, Entete(F, 3));
...
void SupprimSelonForces(char *nomFichier, TableIndex *tabIndex)
{
    ...
    printf("\n\t>> Le numero du grade choisi: ");
    scanf("%d", &forceArmee);
    printf("\n>>   En train de supprimer tous les enregistrements avec tel region, Veuillez patiener SVP!..."); 
    F = Ouvrir(nomFichier, 'A');
    cout = 0;
loop:
    i = 1;
    ...
    while (i != -1)
    {
        ...
        for (j = 0; j < buf->nb; j++)
        {
            ...
            {
                if (b == 0)
                    supprimEnregLnoF(nomFichier, i, j, &cout);
                if (b != 0)
                    supprimEnregLnoF(nomFichier, i, j, &c);
                cpt++;
                RechDichotomique(tabIndex, buf->tab[j].matricule, &trouv, &pos);
                if (pos == tabIndex->NbE - 1)
                {
    ...
    printf("\n\t>> Suppression terminee");
    printf("\n\n\t\t+-----------------------------------------------------------------+");
    printf("\n\t\t| Le cout de cette opeartion est : %-5d accees disque            |", cout/4);
    printf("\n\t\t+-----------------------------------------------------------------+");
...
void Fragmentation(char *nomFichier)
{
  ...
  printf("\n\n\t\t+-----------------------------------------------------------+");
    ...
    if (a == 1)
    {
        ...
        case 1:
            F = Ouvrir("F1.bin", 'A');
            AfficherEntete("F1.bin");
            AffichageBloc("F1.bin", 1);
            break;
        case 2:
            F = Ouvrir("F2.bin", 'A');
            AfficherEntete("F2.bin");
            AffichageBloc("F2.bin", 1);
            break;
        case 3:
            F = Ouvrir("F3.bin", 'A');
            AfficherEntete("F3.bin");
            AffichageBloc("F3.bin", 1);
            break;
        case 4:
            F = Ouvrir("F4.bin", 'A');
            AfficherEntete("F4.bin");
            AffichageBloc("F4.bin", 1);
            break;
        case 5:
            F = Ouvrir("F5.bin", 'A');
            AfficherEntete("F5.bin");
            AffichageBloc("F5.bin", 1);
            break;
        case 6:
            F = Ouvrir("F6.bin", 'A');
            AfficherEntete("F6.bin");
            AffichageBloc("F6.bin", 1);
            break;
        default:
